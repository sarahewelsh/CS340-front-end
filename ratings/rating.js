...
function addRow(addedVals) {
	...
	var table = document.getElementById("ratings");
	...
	newRow.lastChild.addEventListener("click", function (event) {
		window.location.href = "edit.html?id=" + addedVals["rating_id"];
	});
	...
	console.log(addedVals.rating_id);

	console.log(addedVals["rating_id"]);

	console.log(
		"http://flip1.engr.oregonstate.edu:8687/ratings/" + addedVals["rating_id"]
	);
	...
	newRow.lastChild.addEventListener("click", function (event) {
		...
		req.addEventListener("load", function () {
			if (req.status >= 200 && req.status < 400) {
				...
				document.getElementById("ratings").deleteRow(newRow.rowIndex);
			}
		});
...
function bindAddButtons() {
	document
		.getElementById("rating_add")
		.addEventListener("click", function (event) {
			console.log("here");
			// Get new values from form
			var new_vals = {
				ratingType: document.getElementById("new_rating").value,
			};
			...
			addreq.onreadystatechange = function () {
				...
				if (this.readyState === XMLHttpRequest.DONE && this.status === 200) {
					...
					checkreq.open(
						"GET",
						"http://flip1.engr.oregonstate.edu:8687/ratings/" + newRowID,
						true
					);
			...
			addreq.open(
				"POST",
				"http://flip1.engr.oregonstate.edu:8687/ratings/new",
				true
			);
...
function bindSearchButtons() {
	document
		...
		.addEventListener("click", function (event) {
			...
			addreq.open(
				"GET",
				"http://flip1.engr.oregonstate.edu:8687/ratings/type/" + newRating,
				true
			);
